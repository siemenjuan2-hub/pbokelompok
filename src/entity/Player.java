...
public class Player extends Entity {
    ...
    public void update() {
        ...
        if (attack == true && stamina > 2) {
            ...
            if (collisionON == false && keyH.upPressed == true || collisionON == false && keyH.downPressed == true
                    || collisionON == false && keyH.rightPressed == true
                    || collisionON == false && keyH.leftPressed == true) {
                switch (direction) {
                    case "up":
            ...
            if (chargeCounter > 10) {
                ...
                if (isiStamina) {
                    if (keyH.shiftPressed && keyH.upPressed == false && keyH.downPressed == false
    ...
    public void contactMonster(Entity monster) {
        ...
        // Monster masih cooldown
        if (monster.attackCooldown > 0) {
            return;
        }

        int damage = Math.max(0, monster.getAtk() - getDefense());

        if (damage > 0) {
            setHp(getHp() - damage);
        }

        // Mulai cooldown monster
